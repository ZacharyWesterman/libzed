...
// https://stackoverflow.com/a/23683218
...
/**
 * @brief A compile-time CRC32 hash function.
 *
 * This template recursively computes the CRC32 hash of a string at compile time.
 * To use this, call `constexpr auto your_var = HASH32("your_string")` with a string literal.
 * It will also work with strings that aren't known at compile time,
...
struct hash32gen<size, size, dummy> {
	 ...
	 *
	 * @param str The string to hash (not used in this specialization).
	 * @param prev_crc The previous CRC value, default is 0xFFFFFFFF.
	 * @return The computed CRC32 hash.
	 */
	static constexpr hash32 crc32(const char *str, hash32 prev_crc = 0xFFFFFFFF) {
