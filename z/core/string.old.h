...
#ifndef Z_NARROW_ASCII
	#define Z_NARROW_UTF_8 //strings with narrow chars have UTF-8 encoding
#endif
    ...
    namespace core
    {
        ...
        inline string<char>::string(const wchar_t* buffer)
        {
            if (buffer)
            {
                ...
                while (buffer[current_length] != null)
                {
					#ifdef Z_NARROW_UTF_8

					if (buffer[current_length] < 0x80)
					 	current_length++;
					else if (buffer[current_length] < 0x0800)
						current_length += 2;
					else if (buffer[current_length] < 0xFFFF)
						current_length += 3;
					else
						current_length += 4;
                ...
                for (Int i=0; i<current_length; i++)
                {
					...
					char* c = &string_array[i];
					wchar_t chr = buffer[buf_i];

					toUTF_8(c, chr);

					#else

					if (buffer[buf_i] > 0xFF)
						string_array[i] = '?';
					else
        ...
        string<CHAR>::string(const string<CHAR_2>& other)
        {
            ...
            if (sizeof(CHAR_2) < sizeof(CHAR))
			{
				for (Int i=0; i<current_length-1; i++)
				{
					...
					//TODO: read from multibyte string
				}
			}
			{
                for (Int i=0; i<current_length-1; i++)
