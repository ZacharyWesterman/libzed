...
namespace z
{
	namespace core
	{
		...
		template <>
		void string<utf32>::read(inputStream& stream, uint32_t delim, encoding enc)
		{
			character_ct = 0;
			this->increase(4);
		...
		template <>
		void string<utf32>::readln(inputStream& stream, encoding enc)
		{
			character_ct = 0;
			this->increase(4);
		...
		bool string<utf32>::operator==(const string<utf32>& other) const
		{
			...
			size_t fast_sz = (character_ct << 2) / sizeof(size_t);
			size_t slow_bg = (fast_sz * sizeof(size_t)) >> 2;
		...
		bool string<utf32>::operator>(const string<utf32>& other) const
		{
			...
			size_t fast_sz = (max_char << 2) / sizeof(size_t);
			size_t slow_bg = (fast_sz * sizeof(size_t)) >> 2;
